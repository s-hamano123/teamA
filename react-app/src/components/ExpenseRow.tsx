...
export const ExpenseRow = ({
  ...
  const routeText = [expense.fromStation, expense.toStation].filter((v) => v.trim() !== "").join(" ～ ");
  const isMissingFromStation = isInvalid && expense.fromStation.trim() === "";
  const isMissingToStation = isInvalid && expense.toStation.trim() === "";
  const isMissingAmount = isInvalid && expense.amount <= 0;
  ...
  return (
    <tr>
      ...
      <td>
        {isReferenceMode ? (
          <span className="read-only-cell-value">{expense.date}</span>
        ) : (
          <input
            type="date"
            value={expense.date}
      ...
      <td>
        {isReferenceMode ? (
          <span className="read-only-cell-value">{expense.paymentType}</span>
        ) : (
          <select
      ...
      <td>
        {isReferenceMode ? (
          <span className="read-only-cell-value">{routeText}</span>
        ) : (
          <div className="route-field">
            <input
              type="text"
              value={expense.fromStation}
              className={isMissingFromStation ? "validation-missing-field" : undefined}
            ...
            <input
              ...
              className={isMissingToStation ? "validation-missing-field" : undefined}
      ...
      <td>
        {isReferenceMode ? (
          <span className="read-only-cell-value">{expense.amount === 0 ? "" : formatAmount(expense.amount)}</span>
        ) : (
          <input
            type="text"
            value={formatAmountInput(expense.amount)}
            className={isMissingAmount ? "validation-missing-field" : undefined}
      ...
      <td>
        {isReferenceMode ? (
          <span className="read-only-cell-value">{expense.tripType}</span>
        ) : (
          <select
      ...
      <td>
        {isReferenceMode ? (
          <span className="read-only-cell-value">{expense.period === 0 ? "" : expense.period}</span>
        ) : (
          <input
            type="text"
            inputMode="numeric"
      ...
      <td>
        {isReferenceMode ? (
          <span className="read-only-cell-value">{expense.remark}</span>
        ) : (
          <input
            type="text"
      ...
      <td>
        {!isReferenceMode && (
          <div className="button-container">
            {isFirstRow ? (
              <button className="add-button" onClick={onAdd}>追加</button>
            ) : (
